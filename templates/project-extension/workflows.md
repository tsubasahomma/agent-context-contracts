...
| Scope escalation | `[finding, request, blocked validation, or review gate]` | `[route to maintainer, planner, orchestrator, adapter, queue, or omitted reason]` | `[unknown, pending, omitted, or maintainer_confirmed]` | `[local evidence pointer or limitation]` |
...
- Durable output formatting, issue-reference policy, trailers, closure keywords,
  and merge-message policy belong in `output-policy.md` or an adapter-owned
  mapping.

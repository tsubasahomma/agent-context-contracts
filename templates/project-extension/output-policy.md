...
maintainers, tools, or adapters create for this repository.
...
Use generic field names or placeholders when a platform-specific adapter owns
the final template mapping.
...
| Issue references | `[allowed, required, forbidden, adapter-owned, pending, or omitted reason]` | `[unknown, pending, omitted, or maintainer_confirmed]` | `[local evidence pointer or limitation]` |
| Closure keywords | `[allowed, required, forbidden, adapter-owned, pending, or omitted reason]` | `[unknown, pending, omitted, or maintainer_confirmed]` | `[local evidence pointer or limitation]` |
| Trailers | `[allowed, required, forbidden, adapter-owned, pending, or omitted reason]` | `[unknown, pending, omitted, or maintainer_confirmed]` | `[local evidence pointer or limitation]` |
| Merge-message policy | `[squash, merge, release, adapter-owned, pending, or omitted rule]` | `[unknown, pending, omitted, or maintainer_confirmed]` | `[local evidence pointer or limitation]` |
...
## Adapter-Owned Platform Fields

Record local decisions about platform or adapter fields without making them
portable doctrine. If the project does not use an adapter, record `not_required`
or an omitted reason.

| Field or mechanism | Owner layer | Local handling | Status |
| --- | --- | --- | --- |
| `[label, status, checkbox, template field, reviewer request, milestone, release field, or omitted reason]` | `[project extension, selected adapter, platform, maintainer, or unknown]` | `[required, optional, forbidden, adapter-owned, pending, or omitted reason]` | `[unknown, pending, omitted, not_required, or maintainer_confirmed]` |
...
- Local policy MAY specialize exact headings, fields, references, trailers,
  message conventions, body handling, and adapter mapping.
...
- Command text that is meant to be copied or executed should be separated from
  unrelated narrative.
- Adapter-owned fields MUST remain adapter or platform mappings, not durable

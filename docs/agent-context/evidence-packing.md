...
format, selected entrypoint or surface layer, command line, or repository-local
workflow.
...
- **Evidence consumer**: the role, person, agent, process, evaluator, selected
  payload layer, or tool expected to use the pack.
...
- selected-entrypoint, selected-surface, or project-local workflows that choose
  to use a packing tool.
...
formatting belongs in a selected entrypoint, selected surface, or tooling layer
and MUST NOT become portable-core doctrine.
...
| `intended_consumer` | Names the role, person, agent, process, evaluator, selected entrypoint or surface layer, or tool expected to use the pack. |
...
Evidence generated by a tool, model, script, selected entrypoint, or selected
surface MUST be treated as an artifact or evidence source with provenance and
...
Selected entrypoints, selected surfaces, and tools MUST NOT weaken
project-local redaction boundaries. A tool-produced pack that cannot preserve
required redactions MUST be treated as unsafe for the affected surfaces.
...
- selected-entrypoint or selected-surface prompt syntax or configuration;
- local source-surface maps or local secrets policy.

Those details belong to project-local extensions, selected entrypoint or surface
documentation, or tooling owned by the relevant layer.
...
- selected-entrypoint or selected-surface behavior for any context-packing tool;
...
packing behavior should live in selected entrypoint, selected surface, or
tooling layers.

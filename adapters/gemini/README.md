...
This adapter provides an optional Gemini-specific entry point for a consumer
repository. It helps Gemini-oriented instruction files route agents back to the
portable contracts and the consumer-owned project extension without making
Gemini behavior part of the portable core.
...
The Gemini adapter stores payload files under `adapters/gemini/files/**`. When a
consumer explicitly selects this adapter, the payload maps to the following
Gemini destination path:
...
is a Gemini-specific entry point in a consumer repository.

## Installation Boundary

Installation is optional and explicit. The presence of this adapter in a source
package MUST NOT install Gemini payload files by itself.

A sync or adoption process MAY install the payload only when the Gemini adapter
...
adapter-installed managed file. It MAY be updated later only when the Gemini
...
has a file at the Gemini destination path that is not recorded as
...
Installed Gemini entry points should route agents to the current durable context
in this order:
...
The Gemini payload may provide a Gemini-specific routing surface. It MUST NOT
duplicate or replace portable-core doctrine, project-local facts, sync
implementation behavior, or future adapter rules for other tools.

## Payload Content Rules

Gemini payload files MUST remain generic. They MUST NOT include real repository
...
URLs, or assumptions about non-Gemini tools.

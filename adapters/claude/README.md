...
This adapter provides an optional Claude-specific entry point for a consumer
repository. It helps Claude-oriented instruction files route agents back to the
portable contracts and the consumer-owned project extension without making
Claude behavior part of the portable core.
...
The Claude adapter stores payload files under `adapters/claude/files/**`. When a
...
is a Claude-specific entry point in a consumer repository.

## Installation Boundary

Installation is optional and explicit. The presence of this adapter in a source
package MUST NOT install Claude payload files by itself.

A sync or adoption process MAY install the payload only when the Claude adapter
...
adapter-installed managed file. It MAY be updated later only when the Claude
...
has a file at the Claude destination path that is not recorded as
...
Installed Claude entry points should route agents to the current durable context
in this order:
...
The Claude payload may provide a Claude-specific routing surface. It MUST NOT
...
Claude payload files MUST remain generic. They MUST NOT include real repository
...
URLs, or assumptions about non-Claude tools.
